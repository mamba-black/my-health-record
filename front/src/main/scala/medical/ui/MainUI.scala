...
object MainUI {
  def apply(): HtmlElement = {
    ...
    div(idAttr := "test001", mainHeader(), mainContent(routeSignalElement), commandHandler)
  }

  private def mainContent(signalElements: Signal[Element]): HtmlElement = {
    val mainContent = div(className := "main", child <-- signalElements)
    mainContent
  }
          ...
          nav(
            div(
              ...
              div(
                ...
                svg.svg(
                  svg.className := "w-8 -8 text-orange",
