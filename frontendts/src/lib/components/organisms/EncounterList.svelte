...
import EncounterEntity from "$lib/domain/EncounterEntity";
...
let {
  patient,
  encounterEntities,
}: { patient: Patient; encounterEntities: EncounterEntity[] } = $props();

let now = dayjs();
let openEncounter = $state(false);
let selectedEncounterEntity: EncounterEntity | undefined = $state();

let encounterEntityWrappers: {
  encounterEntity: EncounterEntity;
  date: string;
}[] = encounterEntities.map((encounterEntity) => {
  log.info("encounterEntity", encounterEntity);
  let date: string;
  if (now < encounterEntity.date) {
    date = "dentro de " + encounterEntity.date.from(now);
  } else {
    date = encounterEntity.date.from(now);
  }
  return { encounterEntity: encounterEntity, date };
});

function onClick(encounterEntity: EncounterEntity, e: Event) {
  log.info("encounterEntity onClick", encounterEntity);
  if (e instanceof KeyboardEvent && e.key !== "Enter") {
    return;
  }
  e.preventDefault();
  openEncounter = !openEncounter;
  selectedEncounterEntity = encounterEntity;
  goto(`${HISTORY}/${patient.id}/encounters/123/`, {
    state: { encounterEntity: encounterEntity, patient: patient },
  });
  log.debug("isOpen", encounterEntity);
}
</script>
      ...
      {#each encounterEntityWrappers as encounterEntityWrapper}
        <li>
          <a
            ...
            onclick={e => onClick(encounterEntityWrapper.encounterEntity, e)}
            onkeydown={e => onClick(encounterEntityWrapper.encounterEntity, e)}
          >
              ...
              <div class="flex-col justify-center items-center rounded-lg bg-white overflow-hidden shadow-md w-20">
                <div class="bg-blue-500 text-white py-1 px-1">
                  <p class="text-xs font-semibold text-white uppercase tracking-wide text-center">
                    {encounterEntityWrapper.encounterEntity.date.format("MMMM")}
                    <span class="text-gray-300 leading-none">
                      <br>{encounterEntityWrapper.encounterEntity.date.format("YYYY")}
                    </span>
                  </p>
                </div>
                <div class="flex-col justify-center items-center">
                  <p class="text-xs text-gray-400 text-center pt-1 px-4 leading-none">
                    {encounterEntityWrapper.encounterEntity.date.format("dddd")}
                  </p>
                  <p class="font-bold text-black text-center pb-1.5 px-3 leading-none" style="font-size: 18px">
                    {encounterEntityWrapper.encounterEntity.date.day()}
                  </p>
                </div>
              </div>
              ...
              <div class="min-w-0 flex-auto">
                <p class="text-sm font-semibold leading-6 text-gray-900">
                  ...
                  <span class="relative inline-block px-2 py-0 font-semibold text-green-900 leading-tight">
                    ...
                    {#if encounterEntityWrapper.encounterEntity.date > now}
                  ...
                  <span> </span>
                  {#if encounterEntityWrapper.encounterEntity.exams.length > 0}
                    <span class="relative inline-block px-2 py-0 font-semibold text-green-900 leading-tight">
            ...
            <div class="hidden sm:flex sm:flex-col sm:items-end">
              ...
              <p class="mt-1 text-xs leading-5 text-gray-500">
                <time datetime={encounterEntityWrapper.encounterEntity.date}>{encounterEntityWrapper.date}</time>
              </p>
              <!--            // </>}}-->
              <!--            // }-->
            </div>
