...
- **Composición de Cuenta e Identidad de Usuario (`User` -> `Person`)**: `User` representa el límite de autenticación / cuenta del sistema (`id`, `active`, `person`, `provider_info`, `is_owner`). La identidad humana física y su demografía viven estrictamente dentro del recurso de dominio **HL7 FHIR R4 `Person`** (`crates/core/src/domain/fhir/person.rs`), que encapsula `name: HumanName`, `telecom: Vec<ContactPoint>`, `identifier: Option<Identifier>`, `birth_date` y `links: Vec<PersonLink>`. Está estrictamente prohibido aplanar o duplicar los campos de `Person` dentro de DTOs o eventos.
...
- **Arquitectura Orientada a Eventos Asíncrona con Apalis (`apalis-postgres`)**:
  - **Desacoplamiento Estricto**: `crates/user` emite `UserCreatedEvent` conteniendo la entidad completa `Person` (`user_id`, `person: Person`, `create_clinic: bool`).
  - **Procesamiento Reactivo en Segundo Plano**: `crates/administration` suscribe un worker de Apalis (`handle_user_created_event`) respaldado por `apalis-postgres` para garantizar entrega *at-least-once*.
  - **Réplicas Demográficas Autónomas**: El worker inicializa asíncronamente la `Organization` (clínica) y `Practitioner` (médico) si `create_clinic == true`, y crea el expediente réplica de `Patient` local asociando la entidad `Person` para mantener la autonomía operativa de cada clínica.
- **Atención Médica Directa (`Encounter` sin `Appointment` previo)**: De acuerdo a HL7 FHIR R4, la creación de un `Encounter` clínico no exige una cita médica previa (`appointment_id = None`), permitiendo atenciones directas de urgencia o walk-in.
